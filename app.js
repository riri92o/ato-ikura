(function () {
  ...
  function renderCalendar() {
    ...
    for (let index = 0; index < 42; index += 1) {
      ...
      if (totals.usage > 0) button.append(createElement("span", "day-amount usage", `利用 ${formatYen(totals.usage)}`));
      if (totals.cardWithdrawal > 0) button.append(createElement("span", "day-amount card", `カード ${formatYen(totals.cardWithdrawal)}`));
      if (totals.outflow > 0) button.append(createElement("span", "day-amount outflow", `出金 ${formatYen(totals.outflow)}`));
